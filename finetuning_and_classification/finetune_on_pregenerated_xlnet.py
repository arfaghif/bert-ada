...
"""
Ardy's Notes:
- There are only 2 functions and 1 Class
...
def main():
    ...
    parser.add_argument("--xlnet_model", type=str, required=True,
    ...
    # Prepare model
    model = XLNetModel.from_pretrained(args.xlnet_model)
    if args.fp16:
        model.half()
    model.to(device)
    if args.local_rank != -1:
    ...
    """@ardy: train disini? datanya dimana?"""
    model.train()
    log_step = 50
    writeout_step = 20000
    tmp_tr_loss = 0
...
"""@ardyh: Kok kayanya black box gitu yak wkwk, kecuali bagian loggingnya"""